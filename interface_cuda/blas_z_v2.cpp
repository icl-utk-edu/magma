...
/***************************************************************************//**
    Perform matrix-matrix product, \f$ C = \alpha op(A) op(B) + \beta C \f$.
    This is the batched version of ZGEMM, operating on a batch of matrices
    ...
    @param[in]
    dA_array Array of pointers, dimension (batchCount, 1)
             Each is a COMPLEX_16 array on GPU device.
             If transA == MagmaNoTrans, the m-by-k matrix A of dimension (ldda,k), ldda >= max(1,m); \n
             otherwise,                 the k-by-m matrix A of dimension (ldda,m), ldda >= max(1,k).

    @param[in]
    ldda    Leading dimension of each matrix A.

    @param[in]
    dB_array Array of pointers, dimension (batchCount, 1)
             Each is a COMPLEX_16 array on GPU device.
             If transB == MagmaNoTrans, the k-by-n matrix B of dimension (lddb,n), lddb >= max(1,k); \n
             otherwise,                 the n-by-k matrix B of dimension (lddb,k), lddb >= max(1,n).

    @param[in]
    lddb    Leading dimension of each matrix B.

    @param[in]
    beta    Scalar \f$ \beta \f$

    @param[in,out]
    dC_array Array of pointers, dimension (batchCount, 1)
             Each is a COMPLEX_16 array on GPU device.
             The m-by-n matrix C of dimension (lddc,n), lddc >= max(1,m).

    @param[in]
    lddc    Leading dimension of each matrix C.
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    cublasZgemmBatched(
        ...
        (const cuDoubleComplex**) dB_array, int(lddb),
        (const cuDoubleComplex*)&beta,
        (cuDoubleComplex**)dC_array, int(lddc), int(batchCount) );
}

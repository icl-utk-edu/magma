...
void magma_zk_testLocking(unsigned int* locks, int n, sycl::nd_item<3> item_ct1) {
    int id = item_ct1.get_local_id(2) % n;
    bool leaveLoop = false;
    while (!leaveLoop) {
        if (dpct::atomic_exchange<unsigned int,
                                  sycl::access::address_space::generic_space>(
                &(locks[id]), 1u) == 0u) {
            //critical section
            leaveLoop = true;
            dpct::atomic_exchange<unsigned int,
                                  sycl::access::address_space::generic_space>(
                &(locks[id]), 0u);
        }
    }
    ...
    magma_queue_t queue )
{
    ...
    if ( min_nnz > -1 ){ 
        if ( matrices == 1 ){
            ...
            /*
            DPCT1049:508: The work-group size passed to the SYCL kernel may
            ...
            */
            ((sycl::queue *)(queue->sycl_stream()))
                ->submit([&](sycl::handler &cgh) {
                    sycl::accessor<magmaDoubleComplex, 1,
                        ...
                        local_x_acc_ct1(sycl::range<1>(256 /*BLOCKSIZE*/), cgh);
            ...
            sycl::range<3> block(1, blocksize2, blocksize1);
            /*
            DPCT1049:509: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
            ((sycl::queue *)(queue->sycl_stream()))
               ...
               //magma_zbajac_csr_o_ls_kernel<<< grid, block, 0, queue->sycl_stream() >>>
            ...
            sycl::range<3> block(1, blocksize2, blocksize1);
            /*
            DPCT1049:510: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
            ...
            sycl::range<3> block(1, blocksize2, blocksize1);
            /*
            DPCT1049:511: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
            ...
            sycl::range<3> block(1, blocksize2, blocksize1);
            /*
            DPCT1049:512: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
            ...
            sycl::range<3> block(1, blocksize2, blocksize1);
            /*
            DPCT1049:513: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
            /*
            DPCT1042:877: The size of the arguments passed to the SYCL kernel
            ...
            */
            ...
            sycl::range<3> block(1, blocksize2, blocksize1);
            /*
            DPCT1049:514: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
            /*
            DPCT1042:878: The size of the arguments passed to the SYCL kernel
            exceeds the minimum size limit (1024) for a non-custom SYCL device.
            You can get the hardware argument size limit by querying
            info::device::max_parameter_size. You may need to rewrite this code
            if the size of the arguments exceeds the hardware limit.

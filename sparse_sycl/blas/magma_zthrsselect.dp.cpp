...
// magma_zprint_gpu( total_size, 1, val, total_size, queue );
 // magma_dprint_gpu( total_size, 1, ddouble_val, total_size, queue );
    ...
    */
    ((sycl::queue *)(queue->sycl_stream()))->submit([&](sycl::handler &cgh) {
        sycl::accessor<float, 1, sycl::access_mode::read_write,
            ...
            sval_acc_ct1(sycl::range<1>(32 /*BLOCK_SIZE*/), cgh);
    ...
    */
    ...
    // start = magma_sync_wtime( queue );
    // zthreshselect_kernel<<<gridvolta, block, 0, queue->sycl_stream()>>>
    //     (sampling, total_size, subset_size, float_val, thrs[0], thrs1, dummy);
    // // second kernel identifies the largest of these thresholds
    // magma_zreduce_thrs<<<gridvolta2, block1, 0, queue->sycl_stream()>>>
    //     ( thrs1, thrs2 );
    // magma_zreduce_thrs<<<grid4, block1, 0, queue->sycl_stream()>>>
    //      ( thrs2, thrs1 );
    //      
    // end = magma_sync_wtime( queue );

...
/******************************************************************************/
static void
magmablas_ztrsv_packed_small_batched(
...
{
    if     ( n <=  2 )
        trsv_small_batched<magmaDoubleComplex,  2, 1>(uplo, transA, diag, n, dA_array, ldda, dx_array, incx, Ai, Aj, xi, batchCount, queue );
    else if( n <=  4 )
        trsv_small_batched<magmaDoubleComplex,  4, 1>(uplo, transA, diag, n, dA_array, ldda, dx_array, incx, Ai, Aj, xi, batchCount, queue );
    else if( n <=  8 )
        trsv_small_batched<magmaDoubleComplex,  8, 1>(uplo, transA, diag, n, dA_array, ldda, dx_array, incx, Ai, Aj, xi, batchCount, queue );
    else if( n <= 16 )
        trsv_small_batched<magmaDoubleComplex, 16, 1>(uplo, transA, diag, n, dA_array, ldda, dx_array, incx, Ai, Aj, xi, batchCount, queue );
    else if( n <= 32 )
        trsv_small_batched<magmaDoubleComplex, 32, 1>(uplo, transA, diag, n, dA_array, ldda, dx_array, incx, Ai, Aj, xi, batchCount, queue );
    else
        printf("error in function %s: nrowA must be less than 32\n", __func__);
}

/******************************************************************************/
static magma_int_t magma_get_ztrsv_packed_batched_nb(magma_int_t n)
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" void
magmablas_ztrsv_packed_recursive_batched(
        magma_uplo_t uplo, magma_trans_t transA, magma_diag_t diag,
        magma_int_t n,
        magmaDoubleComplex **dA_array, magma_int_t Ai, magma_int_t Aj, magma_int_t ldda,
        magmaDoubleComplex **dx_array, magma_int_t xi, magma_int_t incx,
...
#define dx_array(i)   dx_array, i
    ...
    const int n2 = magma_get_ztrsv_packed_batched_nb(n);
    const int n1 = n - n2;
...
/***************************************************************************//**
    ...
    The vector x is overwritten on b. This version is for a matrix A

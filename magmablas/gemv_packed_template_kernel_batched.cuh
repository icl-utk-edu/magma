    ...
    T beta, T**       y_array, T       * y, int incy, int stridey, int yi )
{
    ...
    gemvn_packed_template_device<T, DIM_X, DIM_Y, TILE_SIZE>
...
/******************************************************************************/
template <typename T, const int DIM_X, const int DIM_Y, const int TILE_SIZE>
void gemvn_packed_template_batched(
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i=0; i<batchCount; i+=max_batchCount) {
        ...
        gemvn_packed_kernel_batched<T, DIM_X, DIM_Y, TILE_SIZE>
    ...
    T beta, T**       y_array, T       * y, int incy, int stridey, int yi)
{
    ...
    gemvc_packed_template_device<T, DIM_X, DIM_Y, TILE_SIZE, trans>
    (m, n, alpha, dA, Ai, Aj, lda, dx, incx, beta, dy, incy);
}
...
/******************************************************************************/
template <typename T, const int DIM_X, const int DIM_Y, const int TILE_SIZE>
void gemvc_packed_template_batched(
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i=0; i<batchCount; i+=max_batchCount) {
        ...
        if (trans == MagmaConjTrans) {
            gemvc_packed_kernel_batched<T, DIM_X, DIM_Y, TILE_SIZE, MagmaConjTrans>
            <<< grid, threads, 0, queue->cuda_stream() >>>
            ( m, n, alpha, dA_array_i, dA+(i*strideA), ldda, strideA, Ai, Aj,
                           dx_array_i, dx+(i*stridex), incx, stridex, xi,
                    beta,  dy_array_i, dy+(i*stridey), incy, stridey, yi);
            ...
            gemvc_packed_kernel_batched<T, DIM_X, DIM_Y, TILE_SIZE, MagmaTrans>
            <<< grid, threads, 0, queue->cuda_stream() >>>
            ( m, n, alpha, dA_array_i, dA+(i*strideA), ldda, strideA, Ai, Aj,
                           dx_array_i, dx+(i*stridex), incx, stridex, xi,
                    beta,  dy_array_i, dy+(i*stridey), incy, stridey, yi);

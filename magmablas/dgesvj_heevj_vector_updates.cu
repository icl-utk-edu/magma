    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    if(nb <= 4) {
        info = gesvj_update_vectors_template_batched_kernel_driver<double, dgesvj_update_nb_4>
        (m, nb, dU0array, lddu0, dU1array, lddu1, dGarray, lddg, heevj_info, heevj_nsweeps, batchCount, queue );
    }
    else if(nb <= 8) {
        info = gesvj_update_vectors_template_batched_kernel_driver<double, dgesvj_update_nb_8>
        (m, nb, dU0array, lddu0, dU1array, lddu1, dGarray, lddg, heevj_info, heevj_nsweeps, batchCount, queue );
    }
    else if(nb <= 16) {
        info = gesvj_update_vectors_template_batched_kernel_driver<double, dgesvj_update_nb_16>
        (m, nb, dU0array, lddu0, dU1array, lddu1, dGarray, lddg, heevj_info, heevj_nsweeps, batchCount, queue );
    }
    else if(nb <= 32) {
        info = gesvj_update_vectors_template_batched_kernel_driver<double, dgesvj_update_nb_32>
        (m, nb, dU0array, lddu0, dU1array, lddu1, dGarray, lddg, heevj_info, heevj_nsweeps, batchCount, queue );
    }
    else {
        info = -100;

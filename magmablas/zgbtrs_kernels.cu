...
#define sB(i, j)  sB[(j)*sldb + (i)]
    ...
    for(int j = 0; j < n/*n1*/; j+=NB) {
        ...
        for(int itx = tx; itx < b_elements_2; itx+=ntx) {
            for(int jb = 0; jb < my_rhs; jb++) {
                sB(tx+b_elements_1, jb) = dB(tx+b_elements_1, jb);
            }
        }
        __syncthreads();
        ...
        for(int ja = 0; ja < NB; ja++) {
           ...
           // apply
           for(int jb = 0; jb < my_rhs; jb++) {
               sB(tx+ja+1, jb) -= rA[ja] * sB(tx+ja,jb)
           }
           __syncthreads();
        ...
        for(int itx = 0; itx < shift_size; itx+=ntx) {
            for(int jb = 0; jb < my_rhs; jb++) {

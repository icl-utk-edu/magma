...
/***************************************************************************//**
    Purpose
    -------
    ZPPSV_INV computes the solution to a complex system of linear equations
    A * X = B, where A is an N-by-N Hermitian positive definite matrix stored in
    packed format and X and B are N-by-NRHS matrices.

    The explicit inverse of 'A' is computed using the Cholesky decomposition:
        A = U**H * U,  if UPLO = 'U', or
        A = L * L**H,  if UPLO = 'L',

     where U is an upper triangular matrix and L is a lower triangular
     matrix.  The inverse of A is then used to solve the system of
     equations A * X = B.
    ...
    ** The routine currently supports a limited range of matrix sizes
    ...
    dAP_array    Array of pointers, dimension (batchCount).
             ...
             is stored in the array AP as follows:
              ...
              On exit, if INFO = 0, the explicit inverse of 'A' is stored using

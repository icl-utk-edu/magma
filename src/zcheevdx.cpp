...
/***************************************************************************//**
    Purpose
    -------
    ZCHEEVDX computes selected eigenvalues and, optionally, eigenvectors
    ...
    magma_int_t *info)
{
    ...
    double *d, *e;
    float *floatd, *floate;
    magma_int_t indfloatA, indfloatd, indfloate;
    ...
    *info = 0;
    ...
    if ( n <= 1 ) {
        ...
        lwmin  = max( n + n*nb + 2*n*n, 2*n + 2*n*n );
        lrwmin = 1 + 7*n + 2*n*n;
        liwmin = 3 + 5*n;
    }
    else {
    ...
    floatA = (magmaFloatComplex*)(work+indfloatA);
    floatd = (float*)(rwork+indfloatd);
    floate = (float*)(rwork+indfloate);
    ...
    timer_stop( time );
    timer_printf( "time cast to float = %6.2f\n", time );
    timer_start( time );

    magma_chetrd(uplo, n, floatA, n, floatd, floate,
                 (magmaFloatComplex*)&work[indtau], (magmaFloatComplex*)&work[indwrk], llwork, &iinfo);
    ...
    magmablas_claset( MagmaFull, n, n, MAGMA_C_ZERO, MAGMA_C_ONE, dfloatQ, ldr, queue );
    magma_cunmtr_gpu( MagmaLeft, uplo, MagmaNoTrans, n, n, dfloatA, ldda, (magmaFloatComplex*)&work[indtau],
                              dfloatQ, ldr, floatA, n, info);
    timer_stop( time );
    ...
    magma_zsetmatrix( n, *m, &work[indwrk + n * (il-1)], n, dX, ldr, queue);

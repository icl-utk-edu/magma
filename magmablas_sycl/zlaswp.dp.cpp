...
typedef struct dpct_type_120352 {
    int npivots;
    int ipiv[MAX_PIVOTS];
} zlaswp_params_t;
...
void zlaswp_kernel(
    ...
    zlaswp_params_t params , sycl::nd_item<3> item_ct1)
{
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        /*
        DPCT1049:1272: The work-group size passed to the SYCL kernel may exceed
        ...
        ((sycl::queue *)(queue->cuda_stream()))
            ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                               zlaswp_kernel(n, dAT(k, 0), ldda, params,
                                             item_ct1);
                           });
    }
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        /*
        DPCT1049:1273: The work-group size passed to the SYCL kernel may exceed
        the limit. To get the device limit, query
        info::device::max_work_group_size. Adjust the work-group size if needed.
        */
        ((sycl::queue *)(queue->cuda_stream()))
            ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                               zlaswpx_kernel(n, dA(k, 0), ldx, ldy, params,
                                              item_ct1);
                           });
    }
...
void zlaswp2_kernel(
    ...
    magmaDoubleComplex *dAT, int ldda,
    int npivots,
    const magma_int_t *d_ipiv, int inci , sycl::nd_item<3> item_ct1)
{
    int tid = item_ct1.get_local_id(2) +
              item_ct1.get_local_range(2) * item_ct1.get_group(2);
    if ( tid < n ) {
    ...
    magma_queue_t queue )
{
    ...
    sycl::range<3> grid(1, 1, magma_ceildiv(n, NTHREADS));
    /*
    DPCT1049:1274: The work-group size passed to the SYCL kernel may exceed the
    ...
    */
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                       [=](sycl::nd_item<3> item_ct1) {
                           zlaswp2_kernel(n, dAT(k1 - 1, 0), ldda, nb, d_ipiv,
                                          inci, item_ct1);
                       });
}

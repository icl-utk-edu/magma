...
void zlacpy_sym_out_full_kernel(
    ...
    magmaDoubleComplex       *dB, int lddb , sycl::nd_item<3> item_ct1)
{
    zlacpy_sym_out_full_device(m, n, dA, ldda, dB, lddb, item_ct1);
}
...
void zlacpy_sym_out_lower_kernel(
    ...
    magmaDoubleComplex       *dB, int lddb , sycl::nd_item<3> item_ct1)
{
    zlacpy_sym_out_lower_device(m, n, rows, perm, dA, ldda, dB, lddb, item_ct1);
}
...
void zlacpy_sym_out_upper_kernel(
    ...
    magmaDoubleComplex       *dB, int lddb , sycl::nd_item<3> item_ct1)
{
    zlacpy_sym_out_upper_device(m, n, dA, ldda, dB, lddb, item_ct1);
}
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        /*
        DPCT1049:1094: The work-group size passed to the SYCL kernel may exceed
        ...
        */
        ((sycl::queue *)(queue->cuda_stream()))
            ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                               zlacpy_sym_out_lower_kernel(m, n, rows, perm, dA,
                                                           ldda, dB, lddb,
                                                           item_ct1);
                           });
    }
        ...
        /*
        DPCT1049:1095: The work-group size passed to the SYCL kernel may exceed
        the limit. To get the device limit, query
        info::device::max_work_group_size. Adjust the work-group size if needed.
        */
        ((sycl::queue *)(queue->cuda_stream()))
            ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                               zlacpy_sym_out_upper_kernel(m, n, dA, ldda, dB,
                                                           lddb, item_ct1);
                           });
    }
    else {
        /*
        DPCT1049:1096: The work-group size passed to the SYCL kernel may exceed
        the limit. To get the device limit, query
        info::device::max_work_group_size. Adjust the work-group size if needed.
        */
        ((sycl::queue *)(queue->cuda_stream()))
            ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                               zlacpy_sym_out_full_kernel(m, n, dA, ldda, dB,
                                                          lddb, item_ct1);
                           });
    }
}

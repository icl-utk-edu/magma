...
{
    output_array[item_ct1.get_group(2)] =
        input + batch_offset[item_ct1.get_group(2)] + row +
        column * lda[item_ct1.get_group(2)];
    //printf("==> kernel_set_pointer input_array %p output_array %p  \n",input+ blockIdx.x * batch_offset,output_array[blockIdx.x]);
}
    ...
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, batchCount),
                       ...
                       [=](sycl::nd_item<3> item_ct1) {
                           kernel_zset_pointer(output_array, input, lda, row,
    ...
    magma_queue_t queue)
{
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, batchCount),
                                         sycl::range<3>(1, 1, 1)),
                       [=](sycl::nd_item<3> item_ct1) {
                           kernel_zset_pointer_var(output_array, input, lda,
                                                   row, column, batch_offset,
                                                   item_ct1);
                       });
}
...
/******************************************************************************/
void zdisplace_pointers_kernel(magmaDoubleComplex **output_array,
               magmaDoubleComplex **input_array, magma_int_t lda,
               magma_int_t row, magma_int_t column, sycl::nd_item<3> item_ct1)
{
    magmaDoubleComplex *inpt = input_array[item_ct1.get_group(2)];
...
void zdisplace_pointers_var_cc_kernel(magmaDoubleComplex **output_array,
               magmaDoubleComplex **input_array, magma_int_t* lda,
               magma_int_t row, magma_int_t column, sycl::nd_item<3> item_ct1)
{
    ...
    if(inpt == NULL || row < 0 || column < 0) 
        ...
        output_array[bid] = &inpt[row + column * lda[item_ct1.get_group(2)]];
}
/******************************************************************************/
// variable leading dimension, constant row offset and variable column offsets
void zdisplace_pointers_var_cv_kernel(magmaDoubleComplex **output_array,
               magmaDoubleComplex **input_array, magma_int_t* lda,
               magma_int_t row, magma_int_t *column, sycl::nd_item<3> item_ct1)
{
    const int bid = item_ct1.get_group(2);
    ...
    if(inpt == NULL || row < 0 || column[bid] < 0) 
        ...
        output_array[bid] =
            &inpt[row + column[bid] * lda[item_ct1.get_group(2)]];
}
/******************************************************************************/
// variable leading dimension, variable row offset and  constant column offsets
void zdisplace_pointers_var_vc_kernel(magmaDoubleComplex **output_array,
               magmaDoubleComplex **input_array, magma_int_t* lda,
               magma_int_t *row, magma_int_t column, sycl::nd_item<3> item_ct1)
{
    const int bid = item_ct1.get_group(2);
    ...
    if(inpt == NULL || row[bid] < 0 || column < 0) 
        ...
        output_array[bid] =
            &inpt[row[bid] + column * lda[item_ct1.get_group(2)]];
}
/******************************************************************************/
// variable leading dimension, variable row and column offsets
void zdisplace_pointers_var_vv_kernel(magmaDoubleComplex **output_array,
               magmaDoubleComplex **input_array, magma_int_t* lda,
               magma_int_t* row, magma_int_t* column, sycl::nd_item<3> item_ct1)
{
    const int bid = item_ct1.get_group(2);
               ...
               magma_int_t batchCount, magma_queue_t queue)
{
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, batchCount),
                                         sycl::range<3>(1, 1, 1)),
                       [=](sycl::nd_item<3> item_ct1) {
                           zdisplace_pointers_kernel(output_array, input_array,
                                                     lda, row, column,
                                                     item_ct1);
                       });
}
...
*/
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, batchCount),
                                         sycl::range<3>(1, 1, 1)),
                       [=](sycl::nd_item<3> item_ct1) {
                           zdisplace_pointers_var_cc_kernel(
...
*/
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, batchCount),
                                         sycl::range<3>(1, 1, 1)),
                       [=](sycl::nd_item<3> item_ct1) {
                           zdisplace_pointers_var_cv_kernel(
                               output_array, input_array, lda, row, column,
                               item_ct1);
                       });
}
...
*/
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, batchCount),
                                         sycl::range<3>(1, 1, 1)),
                       [=](sycl::nd_item<3> item_ct1) {
                           zdisplace_pointers_var_vc_kernel(
                               output_array, input_array, lda, row, column,
                               item_ct1);
                       });
}
...
*/
    ((sycl::queue *)(queue->cuda_stream()))
        ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, batchCount),
                                         sycl::range<3>(1, 1, 1)),
                       [=](sycl::nd_item<3> item_ct1) {
                           zdisplace_pointers_var_vv_kernel(
                               output_array, input_array, lda, row, column,
                               item_ct1);
                       });
}

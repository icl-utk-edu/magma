                         ...
                         sycl::nd_item<3> item_ct1)
{
    ...
    dA += item_ct1.get_group(1) * (mstride + nstride * ldda);
                         ...
                         sycl::nd_item<3> item_ct1)
{
    ...
    if ( i < m ) {
        ...
        /*
        DPCT1064:1336: Migrated make_cuDoubleComplex call is used in a macro
    ...
    magma_queue_t queue )
{
    ...
    sycl::range<3> grid(1, ntile, magma_ceildiv(m, NB));

    //printf( "m %d, grid %d x %d, threads %d\n", m, grid.x, grid.y, threads.x );
    if ( uplo == MagmaUpper ) {
        /*
        DPCT1049:1337: The work-group size passed to the SYCL kernel may exceed
        ...
        */
        ((sycl::queue *)(queue->cuda_stream()))
            ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                               zsymmetrize_tiles_upper(m, dA, ldda, mstride,
                                                       nstride, item_ct1);
                           });
    }
    else {
        /*
        DPCT1049:1338: The work-group size passed to the SYCL kernel may exceed
        the limit. To get the device limit, query
        info::device::max_work_group_size. Adjust the work-group size if needed.
        */
        ((sycl::queue *)(queue->cuda_stream()))
            ->parallel_for(sycl::nd_range<3>(grid * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                               zsymmetrize_tiles_lower(m, dA, ldda, mstride,
                                                       nstride, item_ct1);
                           });
    }
}

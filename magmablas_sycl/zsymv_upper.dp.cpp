...
/***************************************************************************//**
    ...
    when multiplying.
*******************************************************************************/
SYCL_EXTERNAL void zsymv_kernel_U(
    ...
    magmaDoubleComplex *sx_blk, magmaDoubleComplex *sx_jj)
{
    ...
    // Else, partial == 0.
    int partial = (blk == item_ct1.get_group_range(2) - 1 ? (n % NB_X) : 0);

    magmaDoubleComplex psum, psum_t;
    /*
    DPCT1064:1387: Migrated make_cuDoubleComplex call is used in a macro
    ...
    */
    ...
    // quarter_NB_X*(NB_X + 2) = 16*(64 + 2) = 1056
     /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
      // for x[ blk ]
      // for x[ jj ], which cycles over all blocks right of diag
    ...
    /*
    DPCT1065:1370: Consider replacing sycl::nd_item::barrier() with
    ...
    */
    ...
    /*
    DPCT1065:1371: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1064:1388: Migrated make_cuDoubleComplex call is used in a macro
    definition and is not valid for all macro uses. Adjust the code.
    */
    psum = MAGMA_Z_ZERO;
#pragma unroll
    ...
    /*
    DPCT1065:1372: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    sA32(ty2, tx2) = psum;
    /*
    DPCT1065:1373: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1065:1374: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    if ( partial ) {
        ...
        for (int j=0; j < half_NB_X; j += 8) {
            if ( ty2+j + half_NB_X < partial ) {
                ...
                /*
                DPCT1064:1389: Migrated make_cuDoubleComplex call is used in a
    ...
    /*
    DPCT1065:1375: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1065:1376: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1065:1377: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    sA32(ty2, tx2) = psum;
    /*
    DPCT1065:1378: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1065:1379: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    if ( partial ) {
        ...
        for (int j=0; j < half_NB_X; j += 8) {
            if ( ty2+j + half_NB_X < partial ) {
                ...
                /*
                DPCT1064:1390: Migrated make_cuDoubleComplex call is used in a
                macro definition and is not valid for all macro uses. Adjust the
                code.
                */
                sA32(tx2, ty2 + j) = MAGMA_Z_ZERO;
    ...
    /*
    DPCT1065:1380: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1064:1391: Migrated make_cuDoubleComplex call is used in a macro
    definition and is not valid for all macro uses. Adjust the code.
    */
    psum = MAGMA_Z_ZERO;
#pragma unroll
    ...
    /*
    DPCT1065:1381: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    sA32(ty2, tx2) = psum;
    /*
    DPCT1065:1382: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1065:1383: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    sA32(ty2, tx2) = psum_t;
    /*
    DPCT1065:1384: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    /*
    DPCT1065:1385: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();
    ...
    for (int jj = blk + 1; jj < item_ct1.get_group_range(2); ++jj) {
        partial = (jj == item_ct1.get_group_range(2) - 1 ? (n % NB_X) : 0);
        ...
        if ( ty == 0 ) {
            if ( partial == 0 || tx < partial ) {
                ...
                /*
                DPCT1064:1395: Migrated make_cuDoubleComplex call is used in a
                macro definition and is not valid for all macro uses. Adjust the
                code.
                */
        ...
        /*
        DPCT1065:1392: Consider replacing sycl::nd_item::barrier() with
        ...
        */
        ...
        for (int k=0; k < 4; k++) {
            ...
            if ( partial ) {
                ...
                for (int j=0; j < 4; j++) {
                    if ( 4*ty + j + k*quarter_NB_X < partial ) {
                        ...
                        /*
                        DPCT1064:1398: Migrated make_cuDoubleComplex call is
            ...
            /*
            DPCT1065:1396: Consider replacing sycl::nd_item::barrier() with
            sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
            better performance if there is no access to global memory.
            */
            item_ct1.barrier();
            ...
            /*
            DPCT1065:1397: Consider replacing sycl::nd_item::barrier() with
            sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
            better performance if there is no access to global memory.
            */
            item_ct1.barrier();
        ...
        /*
        DPCT1065:1393: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
        ...
        /*
        DPCT1065:1394: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
    ...
    sA16(ty, tx) = total;
    /*
    DPCT1065:1386: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
    performance if there is no access to global memory.
    */
    item_ct1.barrier();

    partial = (blk == item_ct1.get_group_range(2) - 1 ? (n % NB_X) : 0);
...
*******************************************************************************/
SYCL_EXTERNAL void
zsymv_kernel_U_sum(int n, magmaDoubleComplex alpha, int lda,
                   magmaDoubleComplex beta, magmaDoubleComplex *__restrict__ y,
                   int incy, magmaDoubleComplex const *__restrict__ work,
                   sycl::nd_item<3> item_ct1)
{
    int tx = item_ct1.get_local_id(2);
    int blk = item_ct1.get_group(2);
    ...
    if ( ind < n ) {
        work += ind;
        /*
        DPCT1064:1399: Migrated make_cuDoubleComplex call is used in a macro
        definition and is not valid for all macro uses. Adjust the code.
        */

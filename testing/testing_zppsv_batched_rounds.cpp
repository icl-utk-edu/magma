...
int main(int argc, char **argv)
{
    ...
    real_Double_t   gflops, gpu_perf, gpu_time=0., x_time;
    ...
    magma_int_t N, nrhs, nrounds, lda, ldb, ldda, lddb, sizeA, sizeAP, sizeA_batch, sizeAP_batch, sizeB, sizeB_batch;
    magma_int_t ione     = 1;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gflops = (( FLOPS_ZPOTRF( N) + (nrounds * FLOPS_ZPOTRS( N, nrhs )) ) / 1e9) * batchCount;
            ...
            gpu_time = 0.;
            if( opts.version == 1 ) {
                x_time = magma_sync_wtime( opts.queue );
                magma_zpptrf_batched( opts.uplo, N, dAP_array, dinfo_array, batchCount, opts.queue);
                x_time = magma_sync_wtime( opts.queue ) - x_time;
                gpu_time += x_time;

                for(magma_int_t r = 0; r < nrounds; r++) {
                    magma_zsetmatrix( N, nrhs*batchCount, hB, ldb, dB, lddb, opts.queue );

                    x_time = magma_sync_wtime( opts.queue );
                    magma_zpptrs_batched( opts.uplo, N, nrhs, dAP_array, dB_array, lddb, batchCount, queue );
                    x_time = magma_sync_wtime( opts.queue ) - x_time;

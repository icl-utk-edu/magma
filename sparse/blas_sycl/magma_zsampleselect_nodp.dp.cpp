    ...
    ->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                   ...
                   [=](sycl::nd_item<3> item_ct1) {
                       magma_sampleselect::compute_abs(
                           val, gputmp1, total_size, item_ct1);
                   });

    sampleselect_nodp(queue->sycl_stream(), gputmp1, gputmp2, gputree, gpuints, total_size, subset_size, gpuresult);
    ...
    magma_queue_t queue )
{
    ...
    */
    ((sycl::queue *)(queue->sycl_stream()))->submit([&](sycl::handler &cgh) {
        sycl::local_accessor<double, 1> sample_buffer_acc_ct1(
            sycl::range<1>(sample_size), cgh);
        sycl::local_accessor<double, 1> leaves_acc_ct1(
            sycl::range<1>(searchtree_width), cgh);
    ...
    ((sycl::queue *)(queue->sycl_stream()))->submit([&](sycl::handler &cgh) {
        ...
        sycl::local_accessor<int32_t, 1> local_counts_acc_ct1(
            sycl::range<1>(searchtree_width), cgh);
    ...
    ((sycl::queue *)(queue->sycl_stream()))->submit([&](sycl::handler &cgh) {
        sycl::local_accessor<int32_t, 1> sums_acc_ct1(

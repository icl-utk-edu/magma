    ...
    magma_queue_t queue )
{
    ...
    /*
    DPCT1064:798: Migrated make_cuDoubleComplex call is used in a macro
    ...
    if (beta == MAGMA_Z_ZERO) {
        ((sycl::queue *)(queue->sycl_stream()))
            ->parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1) {
                    zgeelltmv_kernel<true>(m, n, nnz_per_row, alpha, dval,
        ...
        ((sycl::queue *)(queue->sycl_stream()))
            ->parallel_for(
                sycl::nd_range<3>(grid * sycl::range<3>(1, 1, threads),
                                  sycl::range<3>(1, 1, threads)),
                [=](sycl::nd_item<3> item_ct1) {
                    zgeelltmv_kernel<false>(m, n, nnz_per_row, alpha, dval,
                                            dcolind, dx, beta, dy, item_ct1);
                });
    }
    ...
    magma_queue_t queue )
{
    ...
    ((sycl::queue *)(queue->sycl_stream()))
        ->parallel_for(sycl::nd_range<3>(grid * sycl::range<3>(1, 1, threads),
                                         sycl::range<3>(1, 1, threads)),
                       [=](sycl::nd_item<3> item_ct1) {
                           zgeelltmv_kernel_shift(m, n, nnz_per_row, alpha,
                                                  tmp_shift, dval, dcolind, dx,

    ...
    if (row < num_rows) {
        ...
        do{
            ...
            if(ilu0col == ilutcol ){
                ...
                /*
                DPCT1064:833: Migrated make_cuDoubleComplex call is used in a
                macro definition and is not valid for all macro uses. Adjust the
                code.
                */
                L_new_val[offsetL + laddL] =
                    MAGMA_Z_ONE + MAGMA_Z_ONE + MAGMA_Z_ONE;
                laddL++;
                ilu0++;
            }
        } while(ilut<endilut && ilu0<endilu0 );
        if (ilu0<endilu0){
            do{
                ...
                L_new_rowidx[ offsetL + laddL ] = row;
                /*
                DPCT1064:834: Migrated make_cuDoubleComplex call is used in a
                macro definition and is not valid for all macro uses. Adjust the
                code.
                */
                L_new_val[offsetL + laddL] =
        ...
        do{
            ...
            if(ilu0col == ilutcol ){
                ...
                /*
                DPCT1064:835: Migrated make_cuDoubleComplex call is used in a
                macro definition and is not valid for all macro uses. Adjust the
                code.
                */
                U_new_val[offsetU + laddU] =
                    MAGMA_Z_ONE + MAGMA_Z_ONE + MAGMA_Z_ONE;
                laddU++;
                ilu0++;
            }
        ...
        if(ilu0<endilu0 ){
            do{
                ...
                U_new_rowidx[ offsetU + laddU ] = row;
                /*
                DPCT1064:836: Migrated make_cuDoubleComplex call is used in a
                macro definition and is not valid for all macro uses. Adjust the
                code.
                */
                U_new_val[offsetU + laddU] =
        ...
        for(int el1=L_row[row]; el1<L_row[row+1]-1; el1++ ){
            ...
            for(int el2 = U_row[ col1 ]+1; el2 < U_row[ col1+1 ]; el2++ ){
                ...
                if(cand_col < row ){
                    ...
                    /*
                    DPCT1064:837: Migrated make_cuDoubleComplex call is used in
                    a macro definition and is not valid for all macro uses.
                    Adjust the code.
                    */
                    L_new_val[offsetL + laddL] =
                        (exist == 0) ? MAGMA_Z_ONE : MAGMA_Z_ZERO;
                    laddL++;
                } else {
                    // check whether this element already exists in U
                    int exist = 0;
                    ...
                    /*
                    DPCT1064:838: Migrated make_cuDoubleComplex call is used in
                    a macro definition and is not valid for all macro uses.
                    Adjust the code.
                    */
                    U_new_val[offsetU + laddU] =
    ...
    magma_queue_t queue )
{
    ...
    */
    ((sycl::queue *)(queue->sycl_stream()))->submit([&](sycl::handler &cgh) {
        ...
        auto L_new_dcol_ct11 = L_new->dcol;
        auto L_new_dval_ct12 = L_new->dval;
        auto U_new_drow_ct14 = U_new->drow;
        auto U_new_drowidx_ct15 = U_new->drowidx;
        auto U_new_dcol_ct16 = U_new->dcol;
        auto U_new_dval_ct17 = U_new->dval;

        cgh.parallel_for(
            sycl::nd_range<3>(grid1 * block1, block1),
            [=](sycl::nd_item<3> item_ct1) {
                zparilut_candidates_insert_1(
                    L0.num_rows, L0.drow, L0.dcol, U0.drow, U0.dcol, L.drow,
                    L.dcol, U.drow, U.dcol, L_new_drow_ct9, L_new_drowidx_ct10,
                    L_new_dcol_ct11, L_new_dval_ct12, insertedL,
                    U_new_drow_ct14, U_new_drowidx_ct15, U_new_dcol_ct16,
